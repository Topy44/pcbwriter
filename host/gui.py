...
class Handler:
    ...
    def on_drawingareaPreview_draw(self, da, cr):
        ...
        # Draw rulers
        self.draw_ruler(da, cr, xpos=20/scale, ypos=0, len=da.get_allocation().width  / scale, scale=scale, vertical=False, skipzero=True)
        self.draw_ruler(da, cr, xpos=0, ypos=20/scale, len=da.get_allocation().height / scale, scale=scale, vertical=True, skipzero=True)


    def draw_ruler(self, da, cr, xpos, ypos, len, scale, offset=0, vertical=False, skipzero=False):
        # Parameters
